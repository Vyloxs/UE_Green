...
int main()
{
	vec vsq[] = {	{0,0}, {20,0}, {20,10}, {0,20},
			{5,5}, {15,5}, {15,15}, {5,15}};


	vec s[] = {
		{0, 0}, {5, 5}, {0, 20}, {5, 15}, {15, 15},{20, 20}, {20, 0}
	};

	vec s1[] = {{6, 0}, {14, 0}, {20, 10}, {14, 20}, {6, 20}, {0, 10}};

	polygon_t sq = { 4, vsq }, /* outer square */
		sq_hole = { 8, vsq }; /* outer and inner square, ie hole */

	polygon_t strange = {7,  s};

	polygon_t hexagon = {6, s1};

	polygon_t* shapes[] = {
		&sq, &sq_hole, &strange, &hexagon
	};


    vec testPoints[] = {
        {10, 10}, {10, 16}, {-20, 10}, {0, 10},
        {20, 10}, {16, 10}, {20, 20}
    };

	for (size_t i = 0; i < 4; i++) {
		for (size_t j = 0; j < sizeof(testPoints) / sizeof(testPoints[0]); j++)  {
			printf("%d\n", inside(testPoints[j], shapes[i], 1e-10));
		}
	}
